...
def jvmfun(p):
    reobj = re.compile(r"-\w+.*?=\w+.*?\s")
    res = reobj.findall(p)
    if isinstance(p, list):  #checking if p is given in list format
        jo = ' '.join(res)
        spl_l = jo.split()
        for num, word in enumerate(spl_l):
            print(f'{num:>3}', f'{word:<}')
        print('\n{:|^30}'.format('frommmmm lissstt'))
    elif isinstance(p, str):  #checking if p is given in string format
        spl_s = p.split()
